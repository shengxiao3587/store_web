...
export default class CheckboxGroup extends Component {
  ...
  onChange(checkedListTemp) {
    let checkedList = checkedListTemp;
    const firstCheck = this.props.options[0].value;
    if (checkedList.length > this.state.checkedList.length && this.state.checkedList.indexOf(firstCheck) === -1) {
      checkedList.unshift(firstCheck);
    } else if (this.state.checkedList.indexOf(firstCheck) > -1 && checkedList.indexOf(firstCheck) === -1) {
      checkedList = [];
    }
  ...
  renderDisabled() {
    ...
    return (
      <div style={{ display: 'flex' }}>
        <span style={{ marginLeft: '16px', width: '120px', display: 'inline-block' }}>
          <span>{label}:</span>
        </span>
        {
          this.state.checkedList.length > 0 ? options.map((item) => {
