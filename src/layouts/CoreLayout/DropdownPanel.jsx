...
class DropdownPanelWrapper extends Component {
  onMenuClick({ key }) {
    if (key === '2') {
    ...
    return (
      <Dropdown overlay={menu} trigger={['click']}>
        <a className="login-name">
          {JSON.parse(localStorage.getItem('name'))}，你好！<Icon type="down" />
        </a>
      </Dropdown>
    );
  }
...
const DropdownPanel = connect(mapStateToProps, mapDispatchToProps)(DropdownPanelWrapper);
